...
// When the gesture is triggered, the button and its parent container will be deleted.
class MyButton: public Button {
	public:
		...
		void tap( Vector pos, int id ) {
			Container* foo = (Container*)(parent);
			foo->remove(this);
			delete foo; 
			delete this;
		}
...
int main( int argc, char* argv[] ) {
	...
	foo->add( new Label( "Container", 180,20, 20,130 ) );

	foo->add( new MyButton( 60,30, -150,90 ) );
	foo->add( new Label( " Button", 200,20, -30,90 ) );
	...
	txt->set( "foobar" );
	foo->add( txt );
	foo->add( new Label( " Textbox", 200,20, 100,-80 ) );
	...
	tile1->add( new Label( "Tile", 80,20,10,0 ) );
	win->add( tile1 );

	Container* tile2 = new Container( 100,100, 180,100, M_PI*50/180, 0, 0xFF );
	tile2->add( new Label( "Subtile", 80,15,5,0 ) );
	foo->add( tile2 );
